...
namespace NeoSharp.Application.Controllers
{
    ...
    public partial class PromptRpcController : IPromptController
    {
        ...
        public Task RpcSendtoaddressCommand(IPEndPoint endPoint, UInt256 asset, UInt160 to, BigInteger value, ulong fee = 0, UInt160 changeAddress = null)
        {
            ...
            {
                ls.Add(changeAddress.ToString(false));
            }

            return this.RpcCallCommand(endPoint, "sendtoaddress", ls.ToArray().ToJson(false));
        }
        ...
        {
            return RpcCallCommand(endPoint, "getblocksysfee", $"[{height.ToString(CultureInfo.InvariantCulture)}]");
        }
        ...
        {
            return RpcCallCommand(endPoint, "getblockhash", $"[{height.ToString(CultureInfo.InvariantCulture)}]");
        }
        ...
        {
            return RpcCallCommand(endPoint, "gettxout", $"[\"{hash.ToString(false)}\",{index.ToString(CultureInfo.InvariantCulture)}]");
        }
        ...
        {
            return RpcCallCommand<Block>(endPoint, "getblock", $"[{height.ToString(CultureInfo.InvariantCulture)}]", deserializeResult);
        }
        ...
        private async Task RpcCallCommand<T>(IPEndPoint endPoint, string method, string parameters = null, bool deserializeResult = false)
        {
            ...
            using (HttpClient wb = new HttpClient())
            {
                var content = new StringContent(
                    "{\"jsonrpc\": \"2.0\", \"method\": \"" + method + "\", \"params\": " + parameters + ", \"id\":1}",
                    Encoding.UTF8,
                    "application/json");

                var endPointUri = new Uri($"http://{endPoint.Address}:{endPoint.Port}");
                var rest = await wb.PostAsync(endPointUri, content);
